...
assert.equal(artifact.release_promotion_execution.format, 'divinity.release_promotion_execution.v1');
assert.equal(artifact.release_promotion_execution.status, 'blocked');
assert.equal(artifact.release_promotion_execution.public_release_ready, false);
assert.equal(artifact.release_promotion_execution.execution_attempted, false);
assert.equal(artifact.release_promotion_execution.command, 'pnpm run release:promotion-execute');
assert.equal(artifact.release_promotion_execution.smoke_test_command, 'pnpm run test:release-promotion-execute');
assert.equal(artifact.release_promotion_execution.confirmation_configured, false);
assert.equal(artifact.release_promotion_execution.registry_publish.token_configured, false);
assert.equal(artifact.release_promotion_execution.binary_upload.token_configured, false);
assert.equal(artifact.release_promotion_execution.binary_upload.release_tag_configured, false);
assert.deepEqual(artifact.release_promotion_execution.blockers, [
  'package_private',
  'non_production_warning',
  'missing_registry_token',
  'missing_github_release_token',
...
assert.deepEqual(artifact.release_promotion_execution.steps.map(step => [step.step_id, step.status]), [
  ['registry_publish', 'skipped'],
  ['binary_attachment_upload', 'skipped']
]);
assert.equal(artifact.release_promotion_execution.does_not_publish_without_clearance, true);
assert.equal(artifact.release_promotion_execution.does_not_upload_without_clearance, true);
assert.equal(artifact.release_promotion_execution.redacts_registry_token, true);
assert.equal(artifact.release_promotion_execution.redacts_github_token, true);
assert.equal(artifact.release_promotion_execution.redacts_release_tag, true);
assert.equal(artifact.release_promotion_execution.redacts_npm_output, true);
assert.equal(artifact.release_promotion_execution.redacts_command_paths, true);
assert.equal(artifact.release_promotion_execution.redacts_signing_secrets, true);
assert.equal(JSON.stringify(artifact.release_promotion_execution).includes(process.cwd()), false);
