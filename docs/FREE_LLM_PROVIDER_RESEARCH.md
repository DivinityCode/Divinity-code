...
- A future live proxy should bind the hosted secret-store adapter boundary to a managed deployment secret service while preserving fail-closed behavior, actor/reason write metadata, and the current redacted readiness/audit surface.
...
- bind the hosted operator secret-store adapter to an approved deployment secret manager while keeping environment variables and the encrypted local file store as local development fallbacks;
