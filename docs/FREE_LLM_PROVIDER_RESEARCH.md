...
- A live proxy can bind the hosted secret-store adapter boundary to a managed deployment secret service through the `external_command` adapter or the provider-specific `aws_secrets_manager` adapter while preserving fail-closed behavior, actor/reason write metadata, and the current redacted readiness/audit surface.
...
- add more provider-specific managed secret-store adapters while keeping the AWS adapter, generic command adapter, environment variables, and the encrypted local file store as deployment/local fallbacks;
