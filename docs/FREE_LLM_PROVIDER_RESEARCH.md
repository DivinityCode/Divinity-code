...
- Provider tool-call approval records now capture per-call approve/reject decisions as `divinity.provider_tool_call_approval.v1` without storing raw tool arguments or executing the tool.
...
- A future live proxy should extend this with fuller request/token budget ledgers, hosted secret integration, and approved tool execution loops that consume approval records while preserving fail-closed behavior.
