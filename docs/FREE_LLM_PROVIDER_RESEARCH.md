...
- `packages/provider-secrets` and API `DIVINITY_PROVIDER_SECRET_REFS_PATH` now provide a reviewed secret-reference bridge for hosted runtime credentials. The manifest stores only provider ids, `secret://` reference ids, and environment variable names; credential values remain in environment-backed local development variables or the encrypted `DIVINITY_PROVIDER_SECRET_STORE_PATH` bootstrap. `POST /provider-secrets/store`, `GET /provider-secrets/readiness`, and provider proxy audit records expose operator write/readiness/usage evidence without resolved secret values.
...
- A future live proxy should replace the encrypted local secret-store bootstrap with a managed operator secret store while preserving fail-closed behavior, actor/reason write metadata, and the current redacted readiness/audit surface.
...
- replace the encrypted local secret-store bootstrap with an approved hosted operator secret store while keeping environment variables as the local development fallback;
