...
Generate the redacted release environment readiness report with:
...
Release environment readiness is a deterministic operator input check, not a release approval. `release:environment-readiness` keeps `public_release_ready: false`, reports whether required release inputs are configured, and stores no registry token values, GitHub token values, release tag values, signing key references, signing identities, signing command paths, native build command paths, local paths, or provider credentials.
