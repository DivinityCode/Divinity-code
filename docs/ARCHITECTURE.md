...
- `packages/provider-secrets` loads optional API runtime secret-reference manifests from `DIVINITY_PROVIDER_SECRET_REFS_PATH`; manifests store only provider ids, `secret://` reference ids, and environment variable names, while real credential values stay in environment-backed operator secret storage. API `GET /provider-secrets/readiness` returns redacted readiness metadata, and API provider route/chat/stream operations record redacted `provider_secret_readiness` and `provider_secret_ref` audit records without credential values.
