...
- GitHub Actions includes a `Release Readiness` workflow at `.github/workflows/release-readiness.yml`. It runs on Node 22 after `npm ci` and covers contracts, public docs, deprecations, provider governance, package metadata, tarball smoke, binary smoke, native binary artifacts, release bundle, release signatures, release promotion, release artifacts, release status, CLI/API smoke, and the full suite before PR integration.
