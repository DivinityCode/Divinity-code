...
## Release Readiness
- `packages/release-artifacts` builds `divinity.release_artifacts.v1` metadata from the package manifest, package file allowlist, redacted Git source provenance, release SBOM metadata, sha256 integrity scan, install paths, release gates, signing readiness, registry publish readiness, binary release readiness, release-candidate bundle readiness, release attestation readiness, release signature artifact readiness, and release promotion preflight metadata.
...
- GitHub Actions includes a `Release Readiness` workflow at `.github/workflows/release-readiness.yml`. It runs on Node 22 after `npm ci` and covers contracts, public docs, deprecations, provider governance, package metadata, tarball smoke, binary smoke, release bundle, release signatures, release promotion, release artifacts, release status, CLI/API smoke, and the full suite before PR integration.
