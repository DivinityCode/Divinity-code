...
- Provider tool execution records consume approved calls through `divinity.provider_tool_execution.v1`, CLI `provider-tool-execute`, and API `GET`/`POST /runs/:id/provider-tool-executions`. Execution requires fresh operator-supplied argument values whose keys exactly match the approval, never persists raw argument values, and currently supports a read-only `read_file` adapter that stores only output summaries and metadata. Unsupported tools are recorded as `blocked`.
