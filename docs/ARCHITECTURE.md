...
- Workspace snapshots exclude `node_modules` and preserve Git metadata.
- Local workspace cleanup is available from `POST /runs/:id/workspace/cleanup`, records a `workspace_cleaned` timeline event, and refuses to delete paths outside the recorded workspace root.
- Remote checkout and containerized runner isolation are still future work.
