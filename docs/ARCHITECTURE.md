...
- Chat execution supports OpenAI-compatible `chat_completions`, Anthropic `anthropic_messages`, and OpenAI `codex_responses` transports. Hosted secret stores, fuller request/token ledgers, and approved live tool execution remain explicit future handlers.
...
- Approval records are audit evidence for future tool execution loops; they do not execute provider-returned tools or reconstruct raw argument values.
