...
## Release Readiness
- `packages/release-artifacts` builds `divinity.release_artifacts.v1` metadata from the package manifest, package file allowlist, redacted Git source provenance, release SBOM metadata, sha256 integrity scan, install paths, release gates, public readiness audit metadata, signing readiness, registry publish readiness, registry publish dry-run readiness, binary release readiness, native binary artifact readiness, signed native binary artifact readiness, GitHub Release binary attachment readiness, release-candidate bundle readiness, release attestation readiness, release signature artifact readiness, release promotion preflight metadata, and guarded release promotion execution metadata.
...
- `pnpm run release:public-readiness-audit` writes `divinity.release_public_readiness_audit.v1` to `dist/release-public-readiness-audit.json`. It records the package privacy and README warning decision state, reuses the release gate blocker inventory, and never publishes, uploads binaries, modifies package metadata, removes warning text, stores local paths, stores release tag values, or stores credential/signing secrets.
- `pnpm run release:environment-readiness` writes `divinity.release_environment_readiness.v1` to `dist/release-environment-readiness.json`. It records redacted configured-state metadata for registry credentials, GitHub Release credentials, release tag, native build command readiness, signing readiness, and explicit public-publish confirmation without executing publish/upload commands or storing token values, tag values, signing refs, identities, command paths, or local paths.
...
- `pnpm run release:registry-dry-run` writes `divinity.release_registry_publish_dry_run.v1` to `dist/release-registry-dry-run.json`. It skips npm execution while blockers remain, and when gates are clear it executes the npm dry-run command without storing registry token values, raw npm output, executable paths, or local absolute paths.
- Binary release readiness is explicit but blocked: release metadata records the local `pnpm run release:binary` build command, `pnpm run test:binary` smoke command, generated Node launcher target filenames, checksum/signing requirements, and blockers without storing local paths, generated binary contents, or signing secrets.
...
- `pnpm run release:native-binary` writes `divinity.release_native_binary_artifacts.v1`, `SHA256SUMS`, and externally built native binary artifacts under `dist/native-binary/` when `DIVINITY_NATIVE_BINARY_BUILD_COMMAND` is configured. The build command receives target metadata over JSON stdin, and public metadata stores only relative paths, checksums, byte counts, blockers, and redaction flags.
- `pnpm run release:signed-native-binary` writes `divinity.release_signed_native_binary_artifacts.v1`, nested native binary artifacts, detached signatures, and `SHA256SUMS` under `dist/signed-native-binary/` when native build and signing inputs are configured. It stores only relative subject paths, checksums, byte counts, blockers, and redaction flags.
- `pnpm run release:binary-attachments` writes `divinity.release_binary_attachment_plan.v1` to `dist/release-binary-attachments.json`. It records the future GitHub Release asset upload command, required signed-native asset sources, `DIVINITY_RELEASE_TAG` readiness, and `GITHUB_TOKEN`/`GH_TOKEN` configured state while never uploading files or storing token values, release tag values, local paths, or signing secrets.
...
- `pnpm run release:signatures` writes `divinity.release_signature_artifacts.v1` under `dist/release-signatures/` with detached local review signatures for release bundle subjects. It passes subject metadata to the configured signing command over JSON stdin and stores only relative paths, checksums, byte counts, redaction flags, and blockers.
- `pnpm run release:promotion-preflight` writes `divinity.release_promotion_preflight.v1` under `dist/release-promotion-preflight.json`, listing required artifacts, release gates, public readiness audit, release environment readiness, registry token readiness, GitHub Release attachment readiness, signing readiness, and blockers without publishing, uploading, or storing secret values.
- `pnpm run release:promotion-execute` writes `divinity.release_promotion_execution.v1` under `dist/release-promotion-execution.json`, blocks by default, and only executes the npm publish plus GitHub Release upload commands when every release blocker is cleared and `DIVINITY_PUBLIC_RELEASE_CONFIRM=publish` is set. The report stores no registry token values, GitHub release token values, release tag values, raw npm output, command paths, local paths, or signing secrets.
- GitHub Actions includes a `Release Readiness` workflow at `.github/workflows/release-readiness.yml`. It runs on Node 22 after `npm ci` and covers contracts, public docs, deprecations, provider governance, package metadata, tarball smoke, public readiness audit, release environment readiness, registry publish dry-run, binary attachment planning, binary smoke, native binary artifacts, signed native binary artifacts, release bundle, release signatures, release promotion preflight, release promotion execution guard, release artifacts, release status, CLI/API smoke, and the full suite before PR integration.

## Capability Discovery
- CLI `capabilities` and API `GET /capabilities` expose `divinity.capabilities.v1`.
- The catalog lists policy presets, runtime adapters, constrained execution adapters, runner isolation profiles, connector adapters, LLM providers, provider secret-store backends, toolsets, and starter recipe summaries from the shared package layer.
...
- Capability discovery keeps extension points explicit so clients do not hard-code supported policies, runtime adapters, execution adapters, connector adapters, providers, provider secret-store backends, or toolsets.
...
- `packages/provider-secrets` loads optional API runtime secret-reference manifests from `DIVINITY_PROVIDER_SECRET_REFS_PATH`; manifests store only provider ids, `secret://` reference ids, and environment variable names. Secret storage goes through an adapter boundary: `DIVINITY_PROVIDER_SECRET_STORE_PATH` plus `DIVINITY_PROVIDER_SECRET_STORE_KEY` selects the default AES-256-GCM local encrypted bootstrap, while `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=external_command` can bind approved deployment secret managers through an absolute executable path and JSON stdin/stdout with no shell interpolation. `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=aws_secrets_manager` binds a provider-specific managed backend through an approved absolute broker command and a `DIVINITY_AWS_SECRETS_MANAGER_SECRET_IDS` JSON map from public `secret://` refs to AWS Secrets Manager secret ids. `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=gcp_secret_manager` binds a Google Cloud Secret Manager backend through an approved absolute broker command and a `DIVINITY_GCP_SECRET_MANAGER_SECRET_IDS` JSON map from public `secret://` refs to GCP secret ids. `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=azure_key_vault` binds an Azure Key Vault backend through an approved absolute broker command and a `DIVINITY_AZURE_KEY_VAULT_SECRET_IDS` JSON map from public `secret://` refs to Azure secret ids. `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=hashicorp_vault` binds a HashiCorp Vault backend through an approved absolute broker command and a `DIVINITY_HASHICORP_VAULT_SECRET_PATHS` JSON map from public `secret://` refs to Vault secret paths. `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=onepassword_secrets_automation` binds a 1Password Secrets Automation backend through an approved absolute broker command and a `DIVINITY_ONEPASSWORD_SECRET_IDS` JSON map from public `secret://` refs to 1Password secret ids. Public backend discovery is exposed through capabilities and `doctor` with backend ids, backend kinds, broker-command requirements, configuration environment variable names, and redaction guarantees only. API `POST /provider-secrets/store` returns only redacted store metadata including backend id/kind, API `GET /provider-secrets/readiness` returns redacted readiness metadata including backend id/kind, and API provider route/chat/stream operations record redacted `provider_secret_write`, `provider_secret_readiness`, and `provider_secret_ref` audit records without credential values, AWS secret ids, GCP secret ids, Azure secret ids, Vault secret paths, or 1Password secret ids.
...
- CLI `doctor` reports provider catalog readiness, provider secret-store backend discovery, toolset catalog readiness, and optional LLM credential readiness without printing or storing secret values.
...
- Toolset resolution now exposes policy permission unions, risk summaries, provider-neutral JSON Schema `tool_schemas`, provider `tool_calls` capability checks, and operator controls such as approval recommendations or provider capability review requirements.
...
- Route planning selects the first authorized provider whose credentials are configured through environment variables, an injected hosted secret resolver, or a no-key local endpoint policy inherited from provider runtime resolution.
...
- `executeProviderProxyChat()`, `executeProviderProxyChatStream()`, CLI `provider-chat`, API `POST /provider-proxy/chat`, and API `POST /provider-proxy/chat/stream` add controlled execution paths behind the same route policy.
- Chat execution supports OpenAI-compatible `chat_completions`, Anthropic `anthropic_messages`, and OpenAI `codex_responses` transports. Hosted runtimes can inject a `credential_resolver` callback, and the API runtime wires one from `DIVINITY_PROVIDER_SECRET_REFS_PATH`, exposing configured secret reference ids for route metadata and returning secret values only inside the runtime process for upstream transport headers.
- The Chat Completions path posts `{ model, messages, max_completion_tokens, temperature, tools }` to `/chat/completions`, with selected tool schemas projected as nested function tools.
- The Anthropic Messages path posts `{ model, system, messages, max_tokens, temperature, tools }` to `/v1/messages` with `anthropic-version: 2023-06-01` and `x-api-key` when credentials are required; selected tool schemas are projected as Anthropic client tools with `input_schema`.
- The OpenAI Responses path posts `{ model, instructions, input, max_output_tokens, temperature, tools }` to `/responses`, with selected tool schemas projected as top-level function tools with JSON Schema `parameters`.
...
- Provider chat execution resolves the selected toolsets before budget checks or upstream requests, projects only those selected tool schemas into upstream provider requests, and keeps raw prompts, request bodies, credentials, and tool arguments out of returned metadata. Route metadata may include `configured_secret_refs`, but never resolver-returned secret values. Providers missing required `tool_calls` capability fail closed with `toolset_resolution` provider capability checks and `provider_capability_review` operator controls.
...
- Provider tool-call approval records attach individual approve/reject decisions to a run through `divinity.provider_tool_call_approval.v1`, CLI `provider-tool-approval`, and API `GET`/`POST /runs/:id/provider-tool-call-approvals`. These records preserve tool call id, provider id, transport, tool name, sorted argument keys, actor, reason, and timestamp while keeping arguments redacted.
- Provider tool execution records consume approved calls through `divinity.provider_tool_execution.v1`, CLI `provider-tool-execute`, and API `GET`/`POST /runs/:id/provider-tool-executions`. Execution requires fresh operator-supplied argument values whose keys exactly match the approval and never persists raw argument values. Current adapters are `read_file`, which stores byte and line-count summaries, `search_files`, which stores only redacted search counts, `list_files`, which stores only redacted file-tree shape counts, and operator-gated `write_file`, which writes full replacement content inside the workspace while blocking traversal, `.git`, and `node_modules` targets. `write_file` can require an approved `expected_sha256` match before writing so stale files fail closed while paths, content, and precondition hashes remain redacted. Operators can attach an optional reviewed `operator_summary`, which is rejected if it contains exact raw argument values. Unsupported tools are recorded as `blocked`.
- Provider chat continuation can accept existing provider tool execution records through `provider_tool_executions` and appends a redacted user context message before the next upstream request. The continuation context includes execution ids, tool call ids, tool names, statuses, adapters, output summaries, optional operator summaries, and safe numeric/boolean metadata only, including `read_file` byte/line counts, `search_files` scan/match counts, `list_files` file/directory/depth counts, and `write_file` byte/line counts plus redaction flags; raw arguments, argument values, file paths, search queries, file contents, filenames, directory names, and raw outputs are not forwarded.
...
- Provider request/token usage is tracked through `createProviderUsageLedger()` as daily provider/model request and token totals only. API/CLI chat calls use a file-backed usage ledger when `DIVINITY_PROVIDER_USAGE_LEDGER_PATH` is configured, return redacted `usage_ledger_record` metadata, and can enforce `usage_budget` caps before upstream calls. Prompts, request bodies, credentials, raw tool arguments, and response text are not written to the ledger.
...
- Streaming returns `divinity.provider_proxy_stream_result.v1` metadata and API `text/event-stream` events with normalized text deltas, redacted tool-call deltas, redacted Anthropic reasoning/signature counts, final output text, usage, and toolset resolution. Raw provider SSE payloads, prompts, request bodies, credentials, raw tool arguments, and thinking/signature content are not forwarded to clients.
