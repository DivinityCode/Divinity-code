...
## Release Readiness
- `packages/release-artifacts` builds `divinity.release_artifacts.v1` metadata from the package manifest, package file allowlist, redacted Git source provenance, release SBOM metadata, sha256 integrity scan, install paths, release gates, signing readiness, registry publish readiness, and binary release readiness.
...
- Binary release readiness is explicit but blocked: release metadata records the future `pnpm run release:binary` build command, `pnpm run test:binary` smoke command, target filenames, checksum/signing requirements, and blockers without storing local paths, generated binaries, or signing secrets.
