...
- `packages/provider-secrets` loads optional API runtime secret-reference manifests from `DIVINITY_PROVIDER_SECRET_REFS_PATH`; manifests store only provider ids, `secret://` reference ids, and environment variable names. `DIVINITY_PROVIDER_SECRET_STORE_PATH` plus `DIVINITY_PROVIDER_SECRET_STORE_KEY` enables an AES-256-GCM local encrypted store bootstrap with actor/reason metadata for writes, while environment variables remain the local fallback. API `POST /provider-secrets/store` returns only redacted store metadata, API `GET /provider-secrets/readiness` returns redacted readiness metadata, and API provider route/chat/stream operations record redacted `provider_secret_write`, `provider_secret_readiness`, and `provider_secret_ref` audit records without credential values.
