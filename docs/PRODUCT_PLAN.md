...
3. Public onboarding and release packaging.
   ...
   - Bootstrap status: GitHub Actions now includes a Node 22 `Release Readiness` workflow with clean `npm ci` setup and contracts, public docs, deprecations, provider, package, tarball, registry dry-run, binary, native binary, signed native binary, release bundle, release promotion, release artifact, release status, smoke, and full-suite gates.
   - Next production slice: add actual published package and public binary attachments after the production warning and `private: true` release gates are cleared.
