...
1. LLM provider runtime configuration.
   ...
   - Bootstrap status: `DIVINITY_PROVIDER_SECRET_STORE_BACKEND=onepassword_secrets_automation` adds a 1Password Secrets Automation adapter through the same approved broker-command contract plus a 1Password secret id map, while keeping 1Password secret ids and resolved values out of public metadata.
...
3. Public onboarding and release packaging.
   ...
   - Bootstrap status: release artifacts now include `divinity.release_gate_clearance.v1` blocker-by-blocker clearance metadata for package privacy, the production warning, registry token readiness, GitHub Release token/tag readiness, native binary distribution, release signing, and GitHub release-readiness evidence while keeping public release blocked.
   - Bootstrap status: `release:public-readiness-audit` now writes `divinity.release_public_readiness_audit.v1` with package privacy and README warning decision evidence, reusing the release blocker inventory while avoiding package metadata edits, warning removal, publishing, uploads, local paths, release tag values, and credential/signing secrets.
   - Bootstrap status: `release:environment-readiness` now writes `divinity.release_environment_readiness.v1` with redacted operator input readiness for registry token, GitHub Release token/tag, native build command, signing inputs, and explicit public-publish confirmation while keeping public release blocked.
   ...
   - Bootstrap status: `release:registry-dry-run` now writes a fail-closed `divinity.release_registry_publish_dry_run.v1` report for the future npm provenance dry-run command, redacting token values, npm output, executable paths, and local paths while publish blockers remain.
   ...
   - Bootstrap status: `release:native-binary` now invokes an operator-configured external native build command and records `divinity.release_native_binary_artifacts.v1` target artifacts plus `SHA256SUMS`, with build-command/path/signing-secret redaction while public downloads remain blocked.
   - Bootstrap status: `release:signed-native-binary` now composes the native build command with the release signing command and records `divinity.release_signed_native_binary_artifacts.v1` detached signatures, checksums, and redacted target metadata while public downloads remain blocked.
   - Bootstrap status: `release:binary-attachments` now writes a fail-closed `divinity.release_binary_attachment_plan.v1` report for future GitHub Release signed-native asset uploads, redacting token values, release tag values, local paths, and signing secrets while upload blockers remain.
   ...
   - Bootstrap status: `release:signatures` now generates local `divinity.release_signature_artifacts.v1` detached signatures for release-candidate bundle subjects through the configured signing command, with checksum metadata and path/signing-secret redaction while public distribution remains blocked.
   - Bootstrap status: `release:promotion-preflight` now generates `divinity.release_promotion_preflight.v1` with required artifacts, release gates, public readiness audit evidence, release environment readiness evidence, registry token readiness, GitHub Release attachment readiness, signing readiness, and blockers before any package publish or signed binary release can proceed.
   - Bootstrap status: `release:promotion-execute` now writes fail-closed `divinity.release_promotion_execution.v1` metadata and refuses to run npm publish or GitHub Release upload commands unless every release blocker clears and `DIVINITY_PUBLIC_RELEASE_CONFIRM=publish` is set, while recording only redacted execution summaries in configured release contexts.
   - Bootstrap status: GitHub Actions now includes a Node 22 `Release Readiness` workflow with clean `npm ci` setup and contracts, public docs, deprecations, provider, package, tarball, public readiness audit, release environment readiness, registry dry-run, binary attachment planning, binary, native binary, signed native binary, release bundle, release promotion preflight, release promotion execution guard, release artifact, release status, smoke, and full-suite gates.
   - Next production slice: execute actual published package and public binary attachment promotion after the production warning, `private: true`, registry token, GitHub release token/tag, native binary, and signing gates are cleared.
