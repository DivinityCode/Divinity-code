...
Provider tool execution recording is exposed through `GET` and `POST /runs/:id/provider-tool-executions`. Posted executions consume an existing approved provider tool-call approval, require fresh `argument_values` whose keys exactly match the approval, and become `divinity.provider_tool_execution.v1` records attached to the run and audit export with type `provider_tool_execution`. Current read-only adapters support `read_file`, `search_files`, and `list_files` from the run workspace, store only redacted output summaries/metadata, accept optional reviewed `operator_summary` text, and block unsupported tools without persisting raw argument values, file paths, filenames, directory names, search queries, or file contents.
