...
Provider tool execution recording is exposed through `GET` and `POST /runs/:id/provider-tool-executions`. Posted executions consume an existing approved provider tool-call approval, require fresh `argument_values` whose keys exactly match the approval, and become `divinity.provider_tool_execution.v1` records attached to the run and audit export with type `provider_tool_execution`. The first adapter supports read-only `read_file` from the run workspace, stores only output summaries/metadata, and blocks unsupported tools without persisting raw argument values or file contents.
