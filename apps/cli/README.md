...
- `run` emits a task payload, generated `run_id`, lifecycle status, preflight decision metadata, provider runtime metadata, toolset resolution metadata with policy permissions, risk summary, provider capability checks, operator controls, durable goal records, budget incident records, connector references, agent activity records, artifact metadata, and a structured event timeline; hard budget cap excess returns `paused`.
...
- `provider-chat --provider openrouter --message "hello"` executes a non-streaming provider request only after route planning succeeds; output omits prompts, request bodies, and credential values. Supported transports are OpenAI-compatible Chat Completions, Anthropic Messages, and OpenAI Responses. Use `--max-completion-tokens` for Chat Completions or `--max-output-tokens` for Responses/Anthropic-style output caps. Use `--toolset`/`--enable-toolset` and `--disable-toolset` to enforce selected toolset compatibility before upstream calls; providers missing required `tool_calls` support fail closed with `toolset_resolution` operator-control metadata. Credentialed providers use their catalog endpoint, while `--base-url` is reserved for no-key local custom development endpoints.
- `toolsets` lists public toolset metadata and the default tool resolution, including policy permission, risk, provider capability, and operator-control metadata when a provider runtime is available on task/run payloads.
