...
- `provider_tool_executions` can be supplied on a later chat call to add redacted continuation context after operator-reviewed execution. The proxy appends a user message containing execution ids, tool call ids, tool names, statuses, adapters, output summaries, and safe numeric/boolean metadata only; raw arguments, argument values, file paths, file contents, raw outputs, prompts, and credentials are not forwarded in this context.
